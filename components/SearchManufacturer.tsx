...
const SearchManufacturer: React.FC<SearchManufacturerProps> = ({selected, setSelected}) => {
    ...
    // filteredManufacturers is a list of manufacturers that match the query
                            ...
                            {filteredManufacturers?.map(item => (
                                <Combobox.Option key={item}
                                    ...
                                    {({selected, active}) => (
                                        <>
                                            <span
                                                className={`block truncate ${selected ? 'font-medium' : 'font-normal'}`}>{item}
                                            </span>
                                            {selected ? (<span
                                                className={`absolute inset-y-0 left-0 flex items-center pl-3 ${active ? 'text-white' : 'text-teal-600'}`}></span>) : null}
                                        </>
                                    )}
                                </Combobox.Option>
                            ))}
